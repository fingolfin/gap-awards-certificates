...
\newfontfamily\handwritingfont{Zapfino}

%---------------------------------------------------------------------------------
%	CERTIFICATE INFORMATION
%---------------------------------------------------------------------------------

\recipient{John Smith} % Recipient name, will be made uppercase automatically



\heading{GAP award} % The main certificate heading at the top, typically will be "Certificate", "Award", "Congratulations" or similar. Will be made uppercase automatically.

\subheading{of excellence} % The subheading to the main heading, useful for qualifying the main heading, leave empty if not required (i.e. \subheading{})


\largedescription{For significant contributions to the GAP computer algebra system.} % The large description line under the recipient name, useful for stating what the certificate is for or writing a congratulatory message, leave empty if not required (i.e. \largedescription{})
...
\leftlinetitle{Date} % The title under the left line, will be made uppercase automatically and can be left empty if not required (i.e. \leftlinetitle{}) in which case the right line will be horizontally centered (if present)
...
\rightlinetitle{Signed} % The title under the right line, will be made uppercase automatically and can be left empty if not required (i.e. \rightlinetitle{}) in which case the left line will be horizontally centered (if present)
\rightlineoptional{}
%\rightlineoptional{on behalf of the GAP Council} % A subtitle to the title under the right line, useful to qualify the title, can be left empty if not required (i.e. \rightlineoptional{})
...
\begin{tikzpicture}
	\node [
		...
		fill=accentcolor, % Background color
...
\vfill
\begin{center}
\includegraphics[height=4cm]{gaplogo.pdf}
\end{center}


\vfill\vfill\vfill\vfill\vfill\vfill % Automatic vertical whitespace for alignment
...
\begin{center}	
	\ifdefempty{\leftlinetitle}{}{ % Display the left line and title(s) if the left title has been entered
		\begin{tikzpicture}
			...
			\node at ($(leftline.south)+(0, 6mm)$) { {\LARGE\today}};
		\end{tikzpicture}%
	}
	...
	\ifdefempty{\rightlinetitle}{}{ % Display the right line and title(s) if the right title has been entered
		\begin{tikzpicture}
			...
			\node at ($(rightline.south)+(0, 6mm)$) { {\LARGE\handwritingfont{}The GAP Council}};
		\end{tikzpicture}%
	}
\end{center}
